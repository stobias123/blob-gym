...
Next, we account for the reward and render if it's time / and rendering is turned on. Then, we update replay memory and finally run a .train(), which will fit our agent.model every step, and then fit our agent.target_model if it has been 5 episodes. Then, we set our current state to the new one, and add one step to our counter:
